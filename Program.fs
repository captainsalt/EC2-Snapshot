...
module EC2 =
    ...
    let private createDescribeInstancesRequest filterName filterValue =
        new DescribeInstancesRequest(
            Filters = new List<Filter>([
                new Filter(filterName, new List<string>([filterValue]))
            ])
        )
    ...
    let rec private  waitForImage (ec2Client: AmazonEC2Client) (imageId: string) = async {
            let describeImageRequest = DescribeImagesRequest(
            ...
            match image with 
            | Some image when image.State.Value = ImageState.Available.Value -> 
    ...
    let getInstanceById (ec2Client: AmazonEC2Client) (instanceId: string) = async {
    ...
    let getInstanceByName (ec2Client: AmazonEC2Client) (nameTag: string) = async {
    ...
    let stopInstance (ec2Client: AmazonEC2Client) (instanceId: string) = async {
        do! ec2Client.StopInstancesAsync(StopInstancesRequest(InstanceIds = ResizeArray [instanceId])) |> Async.AwaitTask |> Async.Ignore
        ...
        let rec waitUntilStopped () = async {
            ...
            | Error s ->
                return Error $"Error while stopping instance '{instanceId}': {s}"
        }

        return! waitUntilStopped ()
    }   

    let createAmi (ec2Client: AmazonEC2Client) (amiRequest: AmiRequest) = async {
        let tags = tagsFromTuple amiRequest.tags
        let imageRequest = new CreateImageRequest(
    ...
    let startInstance (ec2Client: AmazonEC2Client) (instanceId: string) = async {
        do! ec2Client.StartInstancesAsync(StartInstancesRequest(InstanceIds = ResizeArray [instanceId])) |> Async.AwaitTask |> Async.Ignore
        ...
        let rec waitUntilStarted () = async {
            ...
            | Error msg ->
                return Error $"Error when starting instance '{instanceId}': {msg}"
        }
